...
$selectedWeek = isset($_GET['week']) ? min(max(intval($_GET['week']), 1), 20) : $onWeek;
...
<body>
    <div class="container">
        <h1 class="page-title">
            2023级 软件技术1班 第<?=$selectedWeek?>周 课程表
        ...
        <div class="update-time">
            最后更新时间：<?=$updateTime?>
            <br>
            浏览器输入 otoo.top/fzkcb 访问课表
            <br>
            <a href="?style=2<?=!empty($_GET["week"])? "&week=".$_GET["week"] : "" ?>" class="week-btn" style="padding:3px 40px">切换样式</a>
            <a href="?style=0<?=!empty($_GET["week"])? "&week=".$_GET["week"] : "" ?>" class="week-btn" style="padding:3px 15px">去除按钮</a>
        </div>

        <div class="week-buttons">
            <?php for ($i = 1; $i <= 20; $i++): ?>
            <a href="?<?=!empty($_GET["style"])? "&style=".$_GET["style"]."&" : "" ?>week=<?=$i?>" class="week-btn <?=$i==$selectedWeek?'active':''?>" <?=$i==$onWeek?'id="on"':''?>>
                第<?=$i?>周
            </a>
            <?php endfor; ?>
        </div>
